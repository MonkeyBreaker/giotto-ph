...
public:
    ...
    {
        return __atomic_is_lock_free(sizeof(T), obj_);
    }
    ...
    {
        return __atomic_compare_exchange_n(obj_, &expected, desired, true,
                                           atomic_memory_order(success),
                                           atomic_memory_order(failure));
    }
    ...
    }  // TODO: not quite this simple

    bool compare_exchange_strong(T& expected, T desired,
                                 std::memory_order success,
                                 std::memory_order failure) const noexcept
    {
        return __atomic_compare_exchange_n(obj_, &expected, desired, false,
                                           atomic_memory_order(success),
                                           atomic_memory_order(failure));
    }
    ...
    // would be great to have wait and notify, but unclear how to implement them
    // efficiently with __atomic
    // void wait(T old, std::memory_order order = std::memory_order::seq_cst)
    // const noexcept; void wait(T old, std::memory_order order =
    // std::memory_order::seq_cst) const volatile noexcept; void notify_one()
    // const noexcept; void notify_one() const volatile noexcept; void
    // notify_all() const noexcept; void notify_all() const volatile noexcept;
...
template <class T>
class atomic_ref<T, typename std::enable_if<std::is_integral<T>::value>::type>
    : public atomic_ref_base<T>
{
public:
...
public:
    ...
    T* fetch_add(
        ...
        std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return __atomic_fetch_add(obj_, arg * sizeof(T),
    ...
    T* fetch_sub(
        ...
        std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return __atomic_fetch_sub(obj_, arg * sizeof(T),
                                  atomic_memory_order(order));
    }
