...
mrzv::trivial_concurrent_hash_map<Key, T, H, E>::find(Key k)
{
    ...
    while (true) {
        ...
        if (equal(kk, k)) {
            while (value(it) == dummy_value())
                ;  // make sure a value has been written (possibly should add
                   // exponential backoff)
            return it;
...
template <class Key, class T, class H, class E>
std::pair<typename mrzv::trivial_concurrent_hash_map<Key, T, H, E>::iterator,
          bool>
mrzv::trivial_concurrent_hash_map<Key, T, H, E>::insert(value_type x)
{
    Key k = x.first;
